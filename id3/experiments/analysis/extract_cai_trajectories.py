...
def main():
    ...
    cai_data_dir = "/home/yunqi/ideas/ID3_DeepRaccess_CAI_Paper/paper_experiment_results/cai_with_penalty"
    ...
    if not Path(cai_data_dir).exists():
        print(f"❌ 数据目录不存在: {cai_data_dir}")
        return
    

    trajectory_data = extract_trajectories_parallel(cai_data_dir)
    ...
    trajectory_csv_path = output_dir / "trajectory_cai_1000steps.csv"
    detailed_df.to_csv(trajectory_csv_path, index=False)
    
    print(f"✅ CAI轨迹数据已保存: {trajectory_csv_path}")
    ...
    trajectory_json_path = output_dir / "trajectory_cai_data.json"
    ...
    print(f"✅ 原始CAI轨迹数据已保存: {trajectory_json_path}")
    print("\n🎯 CAI轨迹数据准备完成，可以开始生成收敛分析图表!")
