...
print("🔄 加载Access-only轨迹数据...")
# Load the trajectory data from our parallel extraction
df = pd.read_csv('/home/yunqi/ideas/ID3_DeepRaccess_CAI_Paper/paper_experiment_results/figures/trajectory_access_only_1000steps.csv')
...
# Add panel title
fig.text(0.25, 0.92, 'A) All 12 ID3 Variants - Access-only Optimization', fontsize=12, fontweight='bold', ha='center')
...
# Save the figure
output_path = '/home/yunqi/ideas/ID3_DeepRaccess_CAI_Paper/paper_experiment_results/figures/fig_convergence_analysis_access_only.png'
print(f"💾 保存图表到: {output_path}")
plt.savefig(output_path, dpi=300, bbox_inches='tight')
plt.savefig(output_path.replace('.png', '.pdf'), dpi=300, bbox_inches='tight')
plt.close()

print("✅ Access-only收敛分析图表生成完成!")
print(f"🎯 输出文件: fig_convergence_analysis_access_only.png/.pdf")
...
print("\n✨ 图表特色:")
print("- Panel A: 12个独立子图显示每个ID3变体的Access-only收敛轨迹")
